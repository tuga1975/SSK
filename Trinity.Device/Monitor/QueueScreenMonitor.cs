    ...
    public static class QueueScreenMonitor
    {
        ...
        private static void ReportDeviceStatus()
        {
            try
            {
                ...
                dAL_DeviceStatus.Update((int)EnumDeviceIds.QueueScreenMonitor, statuses);
                Trinity.SignalR.Client.Instance.DeviceStatusChanged((int)EnumDeviceIds.QueueScreenMonitor, statuses);
            }
            catch (Exception ex)
            {

    ...
    public class SHPController : ApiController
    {
        ...
        public async System.Threading.Tasks.Task<IHttpActionResult> SHPPostNotification([FromBody] SHPNotificationModel data)
        {
            try
            {
                ...
                {
                    await System.Threading.Tasks.Task.Run(() => Trinity.SignalR.Client.Instance.SendToAppDutyOfficers(null, Subject, data.Content, data.Type, EnumStation.SHP, false, IDNoti));
                    //return Ok(IDNoti);
                    return Ok(true);
                }
            }
